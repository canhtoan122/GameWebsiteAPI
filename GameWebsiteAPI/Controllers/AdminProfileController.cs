...
namespace GameWebsiteAPI.Controllers
{
    ...
    public class AdminProfileController : ControllerBase
    {
        ...
        public AdminProfileController(DataContext context)
        {
            _context = context;
        }

        [HttpGet]
        public async Task<ActionResult<List<AdminProfile>>> Get()
        {
            return Ok(await _context.AdminProfile.ToListAsync());
        }

        [HttpPost]
        public async Task<ActionResult<List<AdminProfile>>> Add(AdminProfile admin)
        {
            _context.AdminProfile.Add(admin);
        ...
        [HttpPut]
        public async Task<ActionResult<List<AdminProfile>>> Update(AdminProfile request)
        {
            var result = await _context.AdminProfile.FindAsync(request.Admin_ID);
            if (result == null)
        ...
        [HttpDelete("{id}")]
        public async Task<ActionResult<List<AdminProfile>>> Delete(int id)
        {
            var result = await _context.AdminProfile.FindAsync(id);
            if (result == null)
                return BadRequest("Admin not found.");

            _context.AdminProfile.Remove(result);
            await _context.SaveChangesAsync();
